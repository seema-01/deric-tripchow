...
//                             "body"  => $message
//                         ],
//                         "data"         => isset($customBodyFields) && !empty($customBodyFields) ? array_map('strval', $customBodyFields) : "",
//                         "android"      => [
//                             "notification" => [
...
function send_notification($fcmMsg, $registrationIDs_chunks, $customBodyFields = [], $title = "test title", $message = "test message", $type = "test type")
{
    ...
    foreach ($registrationIDs_chunks as $chunk) {
        ...
        foreach ($chunk as $registrationID) {
            ...
            $data = [
                "message" => [
                    ...
                    "data" => array_map('strval', $customBodyFields),
                    "android" => [
                        "notification" => [
                            'click_action' => 'FLUTTER_NOTIFICATION_CLICK',
                        ],
        ...
        //     } else {
        //         // Log or inspect the response from FCM
        //         print_r('FCM Response: ' . $result);
        //     }
        //     curl_multi_remove_handle($mh, $ch);
        //     curl_close($ch);
        // }
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        for ($k = 0; $k < count($order_item_data); $k++) {
            ...
            if (!empty($order_item_data)) {
                ...
                // print_r($order_item_data[$k]['add_ons']);
...
// {
//     $t = &get_instance();
//     $user_details = $t->db->select('u.id,username,email,u.mobile,balance,dob,u.image,referral_code, friends_code, c.name as city_name,a.area as area,a.landmark,a.pincode')
...
function get_favorites($user_id, $type = 'products', $limit = NULL, $offset = NULL)
{
    ...
    if ($type == 'products') {
        ...
        // $q = $CI->db->select("f.*,u.username as owner_name ,p.user_id, p.cooking_time,p.slug, p.partner_name, p.type, p.store_url, p.description, p.profile, p.address,p.rating as partner_rating,p.no_of_ratings,p.cooking_time as partner_cook_time,(SELECT MIN(if(pv.special_price != 0, pv.special_price, pv.price)) FROM `partner_data` pd JOIN products p on p.partner_id = pd.user_id JOIN product_variants pv ON pv.product_id=p.id WHERE pd.user_id=pd.user_id ) as price_for_one')")
        ...
        //  (SELECT MIN(if(pv.special_price != 0, pv.special_price, pv.price)) FROM `partner_data` pd JOIN products p on p.partner_id = pd.user_id JOIN product_variants pv ON pv.product_id=p.id WHERE pd.user_id=pd.user_id ) as price_for_one')
...
{
    $t = &get_instance();
    $t->db->select('COUNT(DISTINCT o.id) as total')
          ->from('orders o')
...
function fetch_partners($filter = [], $user_id = null, $limit = NULL, $offset = '', $sort = 'u.id', $order = 'DESC', $search = NULL)
{
    ...
    if ($sort === 'u.id' and $order === 'DESC' and $offset == '') {
        $search_res->order_by('RAND()', '', false);
    } else {
        $search_res->order_by($sort, $order);
    }
    ...
    foreach ($restro_search_res as $row) {
        ...
        if (isset($filter) && !empty($filter['latitude']) && !empty($filter['longitude'])) {
            ...
            $partner_distance = calculate_distance(floatval($filter['latitude']),floatval($filter['longitude']),floatval($row['latitude']),floatval($row['longitude']));

            $tempRow['distance'] = strval(round($partner_distance)) . " km";
        } else {
...
function send_notifications($role_id = "", $role = "user", $title = "", $body = "", $type = "", $id = "")
{
    ...
    if ($role == 'admins') {
        ...
        $user_roles = fetch_details("", "user_permissions up", 'u.fcm_id,u.web_fcm_id', "", "", "", "", "", "", 'users u', 'u.id=up.user_id');
        foreach ($user_roles as $user) {
            $user_res = fetch_details(['id' => $user['user_id']], 'users', 'fcm_id,web_fcm_id');
            // $fcm_ids[0][] = $user_res[0]['fcm_id'];
        ...
        $partner = fetch_details(['u.id' => $role_id], 'users u', 'u.email,rd.partner_name,u.fcm_id,u.web_fcm_id', null, null, null, "DESC", "", '', "partner_data rd", "rd.user_id=u.id");
        if(isset($partner[0]['fcm_id']) && !empty($partner[0]['fcm_id'])){

            $fcm_ids[0][] = $partner[0]['fcm_id'];
        }
        if(isset($partner[0]['web_fcm_id']) && !empty($partner[0]['web_fcm_id'])){

            $fcm_ids[0][] = $partner[0]['web_fcm_id'];
        }
        ...
        $user_res = fetch_details(['id' => $role_id], 'users', 'username,web_fcm_id,fcm_id,email');
...
function update_rider($rider_id, $order_id, $status = "accepted")
{
    ...
    $user_res = fetch_details(['id' => $rider_id], 'users', 'fcm_id,web_fcm_id,username');
    $fcm_ids = array();
    $msg = "";
    if (isset($user_res[0]) && !empty($user_res[0])) {
        ...
        if (isset($current_rider[0]['rider_id']) && $current_rider[0]['rider_id'] == $rider_id) {
            ...
            // $custome_notification = get_settings('custome_notifications', true);
            // if(!empty($custome_notification && $custome_notification['rider_assign_order_mode'] == 1)){
            //     $fcmMsg = array(
            //         'title' => $custome_notification['rider_assign_order_title'],
            //         'body' => $custome_notification['rider_assign_order_message'],
            //         'type' => "order"
            //     );
            //     $msg = 'Rider Notified. ';
