                        ...
                        <form class="form-horizontal form-submit-event" action="<?= base_url('partner/promo_code/add_promo_code'); ?>" method="POST">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    <!-- promo type -->
                                    <!-- <div class="form-group col-md-6">
                                        <label for="">Promocode Type <span class='text-danger text-sm'>*</span></label>
                                        <select name="promocode_type" id="promocode_type" class="form-control">
                                            <option value="">Select</option>
                                            <option value="default" <?= (isset($fetched_details[0]['promocode_type']) && $fetched_details[0]['promocode_type'] == 'default') ? 'selected' : '' ?>>Default</option>
                                            <option value="partner" <?= (isset($fetched_details[0]['promocode_type']) && $fetched_details[0]['promocode_type'] == 'partner') ? 'selected' : '' ?>>Partner</option>
                                        </select>
                                    </div> -->
                                <!-- partner select2 -->
                                        <!-- <div class="form-group col-md-6 partners <?= (isset($fetched_details[0]['promocode_type']) && $fetched_details[0]['promocode_type'] == 'partner') ? '' : 'd-none' ?>">
                                            <label for="cities" class="col-sm-12 col-form-label">Select Partners <span class='text-danger text-sm'>*</span></label>
                                            <div class="col-sm-12">
                                                <select name="partners[]" class=" select_multiple w-100" multiple data-placeholder=" Type to search and select partners">
                                                    <option value=""><?php
                                                    // (isset($partners) && empty($partners)) ? 'No Categories Exist' : 'Select Categories' 
                                                    ?>
                                                    </option>
                                                    <?php
                                                    // $selected_val = (isset($fetched_data[0]['id']) && !empty($fetched_data[0]['id'])) ? $fetched_data[0]['partners'] : '';
                                                    // $selected_vals = explode(',', $selected_val);
                                                    // echo get_partners_option_html($partners, $selected_vals);
