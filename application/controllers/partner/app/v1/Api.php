...
class Api extends CI_Controller
{
    ...
    public function update_order_status()
    {
        ...
        */
        ...
        if (isset($pickup) && !empty($pickup)) {
            if ($pickup[0]['is_self_pick_up'] == "1" && $_POST['status'] != 'cancelled') {
                $this->form_validation->set_rules('owner_note', 'owner_note', 'trim|xss_clean');
                $this->form_validation->set_rules('self_pickup_time', 'self_pickup_time', 'trim|xss_clean');
            }
        }
         if (isset($pickup) && !empty($pickup)) {
            // print_R($pickup);
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            if (!empty($settings)) {
                ...
                // $general_settings['authentication_method'] = ($authentication_settings['authentication_method'] == 'sms') ? 1 : 0;
        ...
        if (!$this->form_validation->run()) {
            ...
            if (isset($_POST['id'])) {
                ...
                $restro_data = array(
                    ...
                    'global_restaurant_time' => $this->input->post('global_restaurant_time', true) == 1 ? "on" : "off",
    ...
    public function update_products()
    {
       ...
       */
        ...
        if (!$this->form_validation->run()) {
            ...
            // $varient_eidt_id = escape_array($varient_eidt_id);
    ...
    //     $this->form_validation->set_rules('table_id', 'Table Id', 'trim|required|xss_clean');
    //     $this->form_validation->set_rules('add_on_id', 'Addon id', 'trim|xss_clean');

    //     $variant_id = $this->input->post('product_variant_id', true);
    //     $qty = $this->input->post('qty', true);

    //     $product_variant = array($variant_id);
    //     $product_qty = array($qty);

    //     $stock = validate_stock($product_variant, $product_qty);
