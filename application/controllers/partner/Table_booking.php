...
class Table_booking extends CI_Controller
{
    ...
    public function dine_in_customers()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_partner() && ($this->ion_auth->partner_status() == 1 || $this->ion_auth->partner_status() == 0)) {
            ...
            // $this->data['tables'] = $this->db->select('dc.*,t.id as table_id,t.title as table_name,t.status as table_status,f.id as floor_id,f.title as floor_name,f.status as floor_status')
            //     ->join('tables t', 'dc.table_id = t.id', 'left')
            //     ->join('floore f ', 't.floore_id=f.id', 'left')
            //     ->where('dc.partner_id', $partner_id)->get('dine_in_cart dc')->result_array();
            ...
            // foreach ($available_tables as $tables) {
            //     $guest =  fetch_details(['table_id' => $tables['table_id']], 'dine_in_cart', 'table_id,guest');

            //     // $this->data['guest'] = $guest[0];
            // }
    ...
    public function get_cart()
    {
        ...
        //     $add_ons_ids = explode(",", $cart_details[$i]['add_on_id']);

        //     for ($k = 0; $k < count($add_ons_ids); $k++) {
        //         $add_ons_details = fetch_details(['id' => $add_ons_ids[$k]], 'product_add_ons', '*');
        //         echo "<pre>";
        ...
        // $cart_details = fetch_details(['table_id' => $_GET['table_id']], 'dine_in_cart', '*');
