    ...
    public function place_order($data)
    {
        ...
        if (!empty($product_variant)) {
            ...
            for ($i = 0; $i < count($product_variant); $i++) {
                ...
                // print_R($product_variant[$i]['partner_id']);
                // $partner_details = fetch_details(['user_id' => $product_variant[$i]['partner_id']],'partner_data','*');
    ...
    public function get_orders_list(
        ...
        foreach ($user_details as $row) {
            if (!empty($row['items'])) {
                ...
                $final_wallet_amount += $row['wallet_balance'];
                ...
                $final_del_amount += intval($row['delivery_charge']);
                ...
                $tempRow['fee'] = $currency_symbol . ' ' . intval($row['tax_amount']);
                $final_charge_amount += intval($row['tax_amount']);
        ...
        if (!empty($user_details)) {
            ...
            $tempRow['name'] = '-';
            $tempRow['mobile'] = '-';
            $tempRow['delivery_charge'] = '<span class="badge badge-danger">' . $currency_symbol . ' ' . $final_del_amount . '</span>';
            $tempRow['items'] = '-';
            $tempRow['partner'] = '-';
            $tempRow['total'] = '<span class="badge badge-danger">' . $currency_symbol . ' ' . $tota_amount . '</span>';
            $tempRow['fee'] = '<span class="badge badge-danger">' . $currency_symbol . ' ' . $final_charge_amount . '</span>';
            $tempRow['wallet_balance'] = '<span class="badge badge-danger">' . $currency_symbol . ' ' . $final_wallet_amount . '</span>';
            $tempRow['discount'] = '-';
