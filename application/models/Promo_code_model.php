...
class Promo_code_model extends CI_Model
{
    ...
    // public function get_promo_codes($limit = "", $offset = '', $sort = 'u.id', $order = 'DESC', $search = NULL, $partner_id = NULL)
    // {
    //     $multipleWhere = '';
    ...
    //         $search_res->or_like($multipleWhere);
    //     }
    //     $where = "(CURDATE() between start_date AND end_date) and status=1 and promocode_type = default";
    //     $search_res->where($where);
    ...
    public function get_promo_codes($limit = "", $offset = '', $sort = 'u.id', $order = 'DESC', $search = NULL, $partner_id = NULL,$from_partner_app = false)
{
    ...
    public function add_promo_code_details($data)
    {
        ...
        // if ($data['promocode_type'] == 'partner') {
        //     $promo_data['partner_id'] = isset($data['partners']) && !empty($data['partners']) ? implode("," , $data['partners'])  : "";
        // }
